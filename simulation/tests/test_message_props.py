...
def test_full_clique():
	...
	for num_nodes in node_nums:
		...
		for j in tqdm(range(10)):
			chance_the.pose_problems()
			for _ in range(3*num_nodes**2):
				chance_the.process_queues()
			for node in chance_the.universe:
				assert len(node.message_queue) == 0, "In round "+str(j)+", though a message queue should be empty, it contains "+str(len(node.message_queue))+" messages of types: "+", ".join(item.message_type for item in node.message_queue)
			for node in chance_the.universe:
